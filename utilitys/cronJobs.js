...
let getAndSetSpecialDealIndexes = async () => {
   ...
   SpecialDeals.remove({}, async err => {
      if (err) {
         console.error(
            'Something wnet wrong when deleteing SpecialDealsIndexes because ' +
               err.message
         );
      } else {
         ...
         SpecialDeals.create(
            ...
            (err, specialDeals) => {
               if (err) {
                  console.error(
                     'Something went wrong trying to create the special deals because ' +
                        err.message
                  );
               } else {
...
async function createFeaturedDeal() {
   ...
   if (saleIDs.length <= 0) {
      ...
      featuredId =
         featuredIds[Math.floor(Math.random() * (featuredIds.length - 1))];
   } else {
...
async function getIdsOfFeaturedNames() {
   ...
   for (let i = 0; i < featureDealNames.length; i++) {
      ...
      let data = await fetch(
         `https://www.cheapshark.com/api/1.0/games?title=${featuredDealName}`
      );
...
async function getFeaturedIDIndexesThatAreOnSale(featuredIds) {
   ...
   for (let i = 0; i < featuredIds.length; i++) {
      ...
      let data = await fetch(
         `https://www.cheapshark.com/api/1.0/deals?id=${featuredId}`
      );
      let deal = await data.json();
...
let getDealAndLength = () => {
   return new Promise((resolve, reject) => {
      fetch('https://www.cheapshark.com/api/1.0/deals', {
         ...
         .catch(err => {
            console.error(
               'error happeoned in get fresh deals fetch because ' + err.message
            );
            reject(-1);
         });
   });
